        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
           <div className="w-full max-w-md bg-[#0f0f0f] border border-red-500/20 rounded-[3rem] p-10 space-y-8 shadow-[0_0_80px_rgba(239,68,68,0.15)] relative overflow-hidden text-center">
              ...
              <div className="space-y-2">
                 ...
                 <p className="text-slate-400 text-xs font-bold uppercase tracking-widest leading-relaxed">Você está prestes a remover permanentemente a Ordem de Serviço de {osToDelete.clientName}.</p>
              </div>
           ...
           <div className="w-full max-w-md bg-[#0f0f0f] border-l border-white/10 shadow-[-20px_0_50px_rgba(0,0,0,0.5)] relative z-10 flex flex-col h-full animate-in slide-in-from-right-full duration-500">
              <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
                 <div className="flex items-center gap-4">
                    ...
                    <div><h2 className="text-xl font-black text-white uppercase tracking-tighter">Central de Alertas</h2><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Pendências Críticas</p></div>
                 </div>
        ...
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-in slide-in-from-top-4 duration-500">
             <div onClick={() => setStatusFilter('Orçamento')} className={`p-6 rounded-3xl border transition-all cursor-pointer group flex flex-col justify-between h-32 ${statusFilter === 'Orçamento' ? 'bg-amber-500/20 border-amber-500/50' : 'bg-white/[0.02] border-white/5'}`}>
                <Clock size={20} className="text-amber-500"/><span className="text-3xl font-black text-white">{stats.orcamento}</span><p className="text-[10px] font-black uppercase text-slate-400">Aguardando Cliente</p>
             </div>
             <div onClick={() => setStatusFilter('Execução')} className={`p-6 rounded-3xl border transition-all cursor-pointer group flex flex-col justify-between h-32 ${statusFilter === 'Execução' ? 'bg-cyan-500/20 border-cyan-500/50' : 'bg-white/[0.02] border-white/5'}`}>
                <Zap size={20} className="text-cyan-500"/><span className="text-3xl font-black text-white">{stats.execucao}</span><p className="text-[10px] font-black uppercase text-slate-400">Em Manutenção</p>
             </div>
             <div onClick={() => setStatusFilter('Pronto')} className={`p-6 rounded-3xl border transition-all cursor-pointer group flex flex-col justify-between h-32 ${statusFilter === 'Pronto' ? 'bg-emerald-500/20 border-emerald-500/50' : 'bg-white/[0.02] border-white/5'}`}>
                <CheckCircle2 size={20} className="text-emerald-500"/><span className="text-3xl font-black text-white">{stats.pronto}</span><p className="text-[10px] font-black uppercase text-slate-400">Retirada Pendente</p>
             </div>
             <div onClick={() => setStatusFilter('Todos')} className={`p-6 rounded-3xl border transition-all cursor-pointer group flex flex-col justify-between h-32 ${statusFilter === 'Todos' ? 'bg-white/10' : 'bg-white/[0.02] border-white/5'}`}>
                <FileText size={20} className="text-white"/><span className="text-3xl font-black text-white">{filteredOrders.length}</span><p className="text-[10px] font-black uppercase text-slate-400">Total no Filtro</p>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {filteredOrders.map(o => (
              <div key={o.id} onClick={() => { setSelectedOS(o); setView('detalhes'); }} className="p-8 rounded-[2.5rem] border border-white/10 bg-white/[0.02] backdrop-blur-md transition-all flex flex-col justify-between min-h-[280px] shadow-xl hover:border-cyan-500/40 group cursor-pointer relative">
                {hasPermission('delete_os') && <button onClick={(e) => handleDeleteOS(e, o)} className="absolute top-6 right-6 p-2.5 bg-red-600/10 text-red-500 opacity-0 group-hover:opacity-100 rounded-xl hover:bg-red-600 hover:text-white transition-all z-20"><Trash2 size={14} /></button>}
                <div className="space-y-6">
                  ...
                  <div className="space-y-3"><h3 className="text-xl font-black text-white group-hover:text-cyan-400 transition-colors uppercase tracking-tight truncate">{o.clientName}</h3><div className="flex items-center gap-3"><span className="text-[11px] font-bold text-slate-300 uppercase tracking-widest">{o.vehicle}</span><span className="text-[11px] font-black text-cyan-400 font-mono tracking-widest bg-cyan-400/5 px-2 py-0.5 rounded border border-cyan-400/10">{o.plate}</span></div></div>
                </div>
