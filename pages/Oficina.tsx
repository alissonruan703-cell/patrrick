    ...
    <div className="pt-24 px-6 lg:px-12 max-w-screen-2xl mx-auto space-y-10 animate-in fade-in duration-700 pb-20">
      ...
      {view === 'lista' && activeTab !== 'nova' && !selectedOS && (
        <div className="flex flex-wrap items-center gap-4 bg-white/[0.01] p-4 rounded-[2rem] border border-white/5 overflow-x-auto no-scrollbar">
           {['Todos', 'Aberto', 'Orçamento', 'Execução', 'Pronto'].map(st => (
             <button key={st} onClick={() => setStatusFilter(st)} className={`px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border flex items-center gap-3 ${statusFilter === st ? 'bg-white text-black border-white' : 'bg-transparent border-white/10 text-slate-500 hover:text-white'}`}>
               {st} {st !== 'Todos' && <span className="bg-white/10 px-2 py-0.5 rounded text-[8px]">{st === 'Aberto' ? stats.aberto : st === 'Orçamento' ? stats.orcamento : st === 'Execução' ? stats.execucao : stats.pronto}</span>}
             </button>
           ))}
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {filteredOrders.map(o => (
              <div key={o.id} onClick={() => { setSelectedOS(o); setView('detalhes'); }} className="p-8 rounded-[2.5rem] border border-white/10 bg-white/[0.02] flex flex-col justify-between min-h-[250px] shadow-xl hover:border-cyan-500/40 cursor-pointer">
                <div className="space-y-6">
                  <div className="flex justify-between items-center"><span className="text-[10px] font-black text-cyan-400 font-mono">#{o.id.slice(-4)}</span><span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase border border-white/10`}>{o.status}</span></div>
                ...
                <div className="pt-6 border-t border-white/10 flex items-center justify-between text-white font-black">R$ {o.total.toFixed(2)}</div>
              </div>
            ))}
          </div>
        </>
        ...
        <div className="max-w-7xl mx-auto space-y-10 animate-in fade-in zoom-in-95">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            <div className="lg:col-span-4 space-y-8">
              ...
              {/* Módulo de Fotos (Campo de Anexar Imagem) */}
            ...
            <div className="lg:col-span-8 space-y-8">
              <div className="bg-white/[0.02] border border-white/10 p-10 lg:p-14 rounded-[3.5rem] space-y-12 shadow-2xl">
                ...
                {/* Formulário de Itens (Campo de Quantidade incluído) */}
