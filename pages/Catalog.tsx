    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-full p-8 space-y-3">
        ...
        <p className="text-sm text-slate-400 font-medium leading-relaxed">
          {isAvailable ? mod.description : "Este módulo está sendo preparado pela nossa equipe de engenharia."}
        </p>
      ...
      {!isAvailable && (
        <div className="absolute top-6 right-6 bg-amber-500/10 backdrop-blur-2xl px-4 py-2 rounded-full border border-amber-500/30 flex items-center gap-2">
          ...
          <span className="text-[10px] font-black text-amber-500 uppercase tracking-[0.2em]">Breve</span>
        </div>
      )}
    </div>
  );
      ...
      <section className="relative min-h-screen flex items-center justify-center px-6 lg:px-20 py-24">
        ...
        <div className="relative z-10 text-center space-y-12 max-w-6xl animate-in fade-in zoom-in duration-1000">
          ...
          <div className="max-w-3xl mx-auto space-y-12">
            <p className="text-slate-300 text-xl lg:text-2xl font-medium leading-relaxed italic px-4">
              "A tecnologia certa para escalar seu negócio multisetorial."
            </p>
            
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              <button 
                onClick={() => navigate('/signup')}
                className="w-full sm:w-auto px-12 py-5 bg-gradient-to-r from-cyan-500 to-violet-600 text-white font-black rounded-2xl hover:brightness-125 hover:scale-105 transition-all text-lg shadow-[0_15px_50px_rgba(0,240,255,0.3)]"
              >
                Começar Agora
              </button>
              <button 
      ...
      <section className="px-6 lg:px-20 py-32 space-y-16">
        <div className="max-w-7xl mx-auto">
          <div className="text-center space-y-4 mb-20">
             <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter">Nosso <span className="text-cyan-400">Catálogo</span></h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
            {systems.map((mod) => (
              <ModuleCard key={mod.id} mod={mod} onNavigate={handleStart} isAvailable={mod.isAvailable} />
            ))}
          </div>
        </div>
      </section>

      <footer className="px-12 py-20 border-t border-white/5 bg-[#0a0a0a] text-center">
         <p className="text-slate-600 text-[10px] font-black uppercase tracking-[0.4em]">© 2025 CRMPlus+ SaaS Management.</p>
      </footer>
    </div>
  );
};
