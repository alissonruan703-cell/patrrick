          ...
          <div className="p-10 lg:p-14 border-b border-white/5 flex flex-col md:flex-row justify-between items-start gap-10">
            <div className="space-y-6">
              <div className="flex items-center gap-6 mb-4">
                {logo ? <img src={logo} className="h-16 w-auto" alt="Logo" /> : <div className="bg-violet-600 w-16 h-16 rounded-[2rem] flex items-center justify-center font-black text-white text-2xl">{osDataFromUrl.companyName[0]}</div>}
               ...
               <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {osDataFromUrl.photos.map((ph: string, i: number) => (
                    <div key={i} className="aspect-square rounded-2xl overflow-hidden border border-white/10 shadow-lg">
                       <img src={ph} className="w-full h-full object-cover" alt={`Serviço ${i}`} />
                    </div>
                  ))}
               </div>
            </div>
