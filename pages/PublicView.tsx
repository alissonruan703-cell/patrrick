...
const PublicView: React.FC = () => {
  ...
  if (!osDataFromUrl) return <div className="min-h-screen flex items-center justify-center p-10 text-slate-500 font-black uppercase tracking-[0.3em]">Orçamento Inválido ou Expirado</div>;
        ...
        <div className="bg-[#0a0a0b]/60 backdrop-blur-3xl rounded-[4rem] border border-white/10 shadow-2xl overflow-hidden">
          <div className="p-10 lg:p-14 border-b border-white/5 flex flex-col md:flex-row justify-between items-start gap-10">
            <div className="space-y-6">
              <div className="flex items-center gap-6 mb-4">
                {logo ? <img src={logo} className="h-16 w-auto object-contain" alt="Logo" /> : <div className="bg-gradient-to-br from-cyan-500 to-violet-600 w-16 h-16 rounded-[2rem] flex items-center justify-center font-black text-white text-2xl shadow-lg">{osDataFromUrl.companyName[0]}</div>}
                <span className="text-2xl font-black text-white uppercase tracking-tighter">{osDataFromUrl.companyName}</span>
              </div>
              <h1 className="text-4xl lg:text-5xl font-black text-white uppercase tracking-tighter">ORÇAMENTO <span className="text-cyan-500">#{osDataFromUrl.id.slice(-4)}</span></h1>
            ...
            <div className="bg-black/40 p-8 rounded-[3rem] border border-cyan-500/20 text-center min-w-[240px] shadow-inner">
               <p className="text-[10px] font-black text-cyan-400 uppercase tracking-widest mb-2">Total do Serviço</p>
          ...
          <div className="p-10 lg:p-14 grid grid-cols-1 md:grid-cols-2 gap-10">
            <div className="p-8 bg-black/40 rounded-[2.5rem] border border-white/5 space-y-6">
               <div className="space-y-1"><p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Proprietário</p><p className="text-xl font-black text-white uppercase">{osDataFromUrl.client}</p></div>
            ...
            <div className="p-8 bg-cyan-500/5 border border-cyan-500/10 rounded-[2.5rem] flex flex-col justify-center space-y-4">
              <p className="text-[10px] font-black text-cyan-500/50 uppercase tracking-widest">Relato Técnico</p>
          ...
          {osDataFromUrl.photos.length > 0 && (
            <div className="px-10 lg:px-14 py-8 space-y-6">
               <div className="flex items-center gap-3 px-2 border-l-4 border-cyan-500"><ImageIcon className="text-cyan-500" size={18}/><h3 className="text-[11px] font-black text-white uppercase tracking-widest">Evidências do Serviço</h3></div>
            ...
            <div className="bg-black/40 rounded-[2.5rem] border border-white/5 overflow-hidden">
               <table className="w-full">
                 <thead className="bg-white/5 text-[9px] font-black text-slate-500 uppercase tracking-widest">
                   <tr><th className="px-8 py-5 text-left">Item / Descrição</th><th className="px-8 py-5 text-center">Qtd</th><th className="px-8 py-5 text-right">Subtotal</th></tr>
                 </thead>
            ...
            <div className="pt-10 flex flex-col sm:flex-row gap-6">
              {currentStatus === 'pending' ? (
                <>
                  ...
                  <button onClick={() => handleClientAction('Execução')} className="flex-1 py-6 bg-emerald-600 text-white font-black rounded-3xl uppercase text-[11px] tracking-widest shadow-[0_10px_30px_rgba(16,185,129,0.3)] hover:brightness-110 active:scale-95 transition-all">Aprovar e Iniciar Serviço</button>
                </>
              ) : (
                <div className={`w-full py-10 text-center font-black rounded-[2.5rem] uppercase text-sm tracking-[0.2em] flex flex-col items-center gap-4 border-2 ${currentStatus === 'approved' ? 'bg-emerald-600/10 text-emerald-500 border-emerald-500/20' : 'bg-red-600/10 text-red-500 border-red-500/20'}`}>
                  {currentStatus === 'approved' ? (
                    <>
                      ...
                      <p className="text-[10px] opacity-60 normal-case font-medium">Nossa equipe já foi notificada e iniciará o serviço em breve.</p>
                    </>
                  ) : (
                    <>
                      <AlertCircle size={48} />
                      Orçamento Recusado.
                      <p className="text-[10px] opacity-60 normal-case font-medium">Entre em contato conosco se houver dúvidas sobre os itens listados.</p>
                    </>
                  )}
                </div>
              )}
