  ...
  return (
    <div className="p-8 pb-32">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-12">
        <div>
          <h1 className="text-4xl font-black uppercase tracking-tighter">Sua Gestão por <span className="text-red-600">Assinatura</span></h1>
          <p className="text-zinc-500 font-bold uppercase text-xs tracking-widest mt-2">Empresa Ativa: {user.companyName}</p>
        </div>
        <button onClick={() => navigate('/billing')} className="bg-zinc-900 border border-zinc-800 px-6 py-3 rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-zinc-800 transition-all flex items-center gap-2">
          Contratar Novos Sistemas <ArrowRight size={14} />
        </button>
      </div>
      ...
      <section className="mb-16">
        <h2 className="text-xs font-black uppercase text-zinc-600 tracking-[0.3em] mb-6 flex items-center gap-3">
          <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div> Serviços Ativos
        </h2>
            ...
            return (
              <div key={sub.id} className="bg-zinc-900 border border-zinc-800 rounded-[2.5rem] p-8 group hover:border-red-600/30 transition-all relative overflow-hidden">
                ...
                <div className="flex gap-3">
                  <button onClick={() => navigate(`/module/${sub.id}`)} className="flex-1 bg-white text-black py-4 rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-zinc-200 transition-all">Entrar</button>
                  <button className="px-5 bg-zinc-800 text-zinc-400 py-4 rounded-2xl font-black uppercase text-xs hover:text-white transition-all">Dados</button>
                </div>
              </div>
            );
          })}
