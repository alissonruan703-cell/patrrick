...
const AdminMaster: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!isMaster) { navigate('/login'); return; }
  ...
  const handleSave = () => {
    ...
    if (editingId) { updated = licenses.map(l => l.id === editingId ? { ...l, ...formData } : l); }
    else { const newLicense: AccountLicense = { id: Date.now().toString(), ...formData, createdAt: new Date().toLocaleDateString('pt-BR') }; updated = [...licenses, newLicense]; }
    setLicenses(updated);
    ...
    <div className="min-h-screen bg-[#0a0c10] text-slate-300 p-6 lg:p-12">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        ...
        <div className="flex gap-4">
          ...
          <button onClick={() => { setEditingId(null); setShowModal(true); }} className="px-8 py-3 bg-violet-600 text-white rounded-xl font-black uppercase tracking-widest text-[10px] shadow-xl shadow-violet-600/20">Nova Licença</button>
        </div>
      </div>
      <div className="max-w-7xl mx-auto bg-[#161920] rounded-3xl border border-white/5 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-black/20 text-[10px] font-black uppercase tracking-widest text-slate-500">
            <tr><th className="px-8 py-6">Empresa / Modulos</th><th className="px-8 py-6">Acesso</th><th className="px-8 py-6">Status</th><th className="px-8 py-6 text-right">Ações</th></tr>
          </thead>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/95 backdrop-blur-sm animate-in fade-in duration-300">
           <div className="w-full max-w-lg bg-[#1a1d23] border border-white/10 rounded-[2.5rem] p-10 space-y-8 relative overflow-y-auto max-h-[90vh]">
              <div className="text-center"><h3 className="text-2xl font-black text-white uppercase">{editingId ? 'Editar' : 'Emitir'} Licença</h3></div>
              <div className="space-y-4">
                 <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1"><label className="text-[9px] font-black text-slate-500 uppercase">Empresa</label><input value={formData.companyName} onChange={e => setFormData({...formData, companyName: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-white text-sm" /></div>
                   <div className="space-y-1"><label className="text-[9px] font-black text-slate-500 uppercase">Responsável</label><input value={formData.fullName} onChange={e => setFormData({...formData, fullName: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-white text-sm" /></div>
                 </div>
                 <div className="space-y-1"><label className="text-[9px] font-black text-slate-500 uppercase">Email</label><input value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-white text-sm" /></div>
                 <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1"><label className="text-[9px] font-black text-slate-500 uppercase">Usuário</label><input value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-white text-sm" /></div>
                   <div className="space-y-1"><label className="text-[9px] font-black text-slate-500 uppercase">Senha</label><input value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-white text-sm" /></div>
                 </div>
                 <div className="space-y-3 p-4 bg-black/40 rounded-2xl border border-white/5">
                    ...
                    <div className="grid grid-cols-2 gap-2">{availableModules.map(m => (<button key={m.id} onClick={() => toggleModule(m.id)} className={`p-3 rounded-xl border text-[9px] font-black uppercase transition-all ${formData.allowedModules.includes(m.id) ? 'bg-violet-600 border-violet-500 text-white' : 'bg-transparent border-white/5 text-slate-500'}`}>{m.name}</button>))}</div>
                 </div>
                 <div className="space-y-1"><label className="text-[9px] font-black text-slate-500 uppercase">Expiração</label><input type="date" value={formData.expirationDate} onChange={e => setFormData({...formData, expirationDate: e.target.value})} className="w-full bg-black/40 border border-white/5 rounded-xl px-4 py-3 text-white text-sm" /></div>
              </div>
              <div className="flex gap-4"><button onClick={() => setShowModal(false)} className="flex-1 py-4 bg-white/5 text-slate-500 rounded-xl font-black uppercase text-[10px]">Cancelar</button><button onClick={handleSave} className="flex-1 py-4 bg-violet-600 text-white rounded-xl font-black uppercase text-[10px]">Salvar Licença</button></div>
           </div>
        </div>
      )}
    </div>
