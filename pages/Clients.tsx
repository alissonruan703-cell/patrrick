...
const ClientsList = () => {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      const key = `crmplus_clients_${savedUser.id}`;
  ...
  const handleAdd = (e: React.FormEvent) => {
    ...
    const key = `crmplus_clients_${currentUser.id}`;
    ...
    <div className="p-8 max-w-7xl mx-auto animate-in fade-in duration-500">
      <div className="flex justify-between items-center mb-12">
        <div className="flex items-center gap-4">
           ...
           <h1 className="text-3xl font-black uppercase tracking-tighter">Clientes <span className="text-red-600">Base {currentUser?.companyName}</span></h1>
        </div>
      ...
      <div className="relative group max-w-xl mb-12">
        ...
        <input placeholder="Busca exclusiva na sua base..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 rounded-3xl py-5 pl-16 pr-6 text-sm font-bold focus:border-red-600 outline-none transition-all" />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {filtered.length === 0 && (
          <div className="col-span-full py-20 text-center opacity-20 border-2 border-dashed border-zinc-800 rounded-[3rem]">
             <p className="font-black uppercase tracking-[0.3em]">Nenhum cliente cadastrado nesta conta</p>
          </div>
        )}
      </div>
      ...
      {showAdd && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-6 backdrop-blur-md animate-in fade-in">
          <div className="bg-zinc-900 border border-zinc-800 p-12 rounded-[3.5rem] w-full max-w-lg shadow-2xl">
            <h2 className="text-2xl font-black uppercase mb-8 border-b border-zinc-800 pb-6">Novo Cadastro Privado</h2>
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div className="pt-6 flex gap-4">
                 ...
                 <button type="submit" className="flex-[2] bg-red-600 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:brightness-110 active:scale-95 transition-all">Salvar na minha base</button>
              </div>
            </form>
          </div>
        </div>
...
const ClientDetails = () => {
  ...
  useEffect(() => {
    ...
    if (savedUser) {
      ...
      const ordersKey = `crmplus_oficina_orders_${savedUser.id}`;
  ...
  if (!client) return (
    <div className="p-32 text-center opacity-10 space-y-4">
       ...
       <p className="font-black uppercase tracking-[0.4em]">Registro não encontrado na sua conta</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-4 bg-zinc-900 border border-zinc-800 p-10 rounded-[3.5rem] shadow-2xl space-y-10 relative overflow-hidden">
          ...
          <div className="pt-8 border-t border-zinc-800 space-y-4">
             <p className="text-[9px] font-black uppercase text-zinc-600 tracking-widest">Informações Privadas</p>
          ...
          <div className="flex-1">
            {activeTab === 'timeline' ? (
              <div className="space-y-8">
                {history.length > 0 ? history.map(ev => (
                  ...
                  <div className="py-32 text-center opacity-10 space-y-6">
                     ...
                     <p className="text-sm font-black uppercase tracking-[0.4em]">Nenhuma atividade privada para exibir</p>
                  </div>
                )}
              </div>
            ) : activeTab === 'oficina' ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {orders.length > 0 ? orders.map(o => (
                  ...
                  <div className="col-span-full py-32 text-center opacity-10 space-y-6">
                     ...
                     <p className="text-sm font-black uppercase tracking-[0.4em]">Nenhuma O.S. encontrada para este cliente</p>
                  </div>
                )}
              </div>
            ) : (
              <div className="py-32 text-center opacity-10 space-y-6">
                 ...
                 <p className="text-sm font-black uppercase tracking-[0.4em]">Aguardando dados de Orçamentos Privados</p>
              </div>
            )}
          </div>
        </div>
